...
- (IBAction)buttonPressed:(id)sender {
	...
	if (sender == basePasswordButton) {
		...
		[panel beginSheet:pwdSheet completionHandler:^(NSModalResponse returnCode) {
			if (returnCode == NSModalResponseOK) {
				// パスワード値変更
				NSString* newPwd = [pwdSheetNewPwdField1 stringValue];
				if ([newPwd length] > 0) {
					[Config sharedConfig].password = [NSString stringWithCString:crypt([newPwd UTF8String], "IP") encoding:NSUTF8StringEncoding];
				} else {
	...
	// パスワード変更シート変更（OK）ボタン
		...
		[panel endSheet:pwdSheet returnCode:NSModalResponseOK];
	}
	// パスワード変更シートキャンセルボタン
	else if (sender == pwdSheetCancelButton) {
		[panel endSheet:pwdSheet returnCode:NSModalResponseCancel];
	}
	// ブロードキャストアドレス追加ボタン（シートオープン）
		...
		// シート表示
		[panel beginSheet:bcastSheet completionHandler:^(NSModalResponse returnCode) {
			if (returnCode == NSModalResponseOK) {
	...
	// ブロードキャストシートOKボタン
		...
		[bcastSheetErrorLabel setStringValue:@""];
		[panel endSheet:bcastSheet returnCode:NSModalResponseOK];
	}
	// ブロードキャストシートキャンセルボタン
	else if (sender == bcastSheetCancelButton) {
		[panel endSheet:bcastSheet returnCode:NSModalResponseCancel];
	}
	// 不在追加ボタン／編集ボタン
	...
	// 不在シートOKボタン
		...
		[(AppControl*)[NSApp delegate] buildAbsenceMenu];
		[panel endSheet:absenceSheet returnCode:NSModalResponseOK];
	}
	// 不在シートCancelボタン
	else if (sender == absenceSheetCancelButton) {
		[panel endSheet:absenceSheet returnCode:NSModalResponseCancel];
	}
	// 通知拒否追加ボタン／編集ボタン
...
// broadcast entry?
	...
	// 通知拒否シートOKボタン
		...
		[refuseTable reloadData];
		[panel endSheet:refuseSheet returnCode:NSModalResponseOK];
	}
	// 通知拒否シートCancelボタン
	else if (sender == refuseSheetCancelButton) {
		[panel endSheet:refuseSheet returnCode:NSModalResponseCancel];
	}
