...
- (id)init {
	...
	if (bind(serverSock, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
		...
		alert.informativeText = [NSString stringWithFormat:NSLocalizedString(@"Err.TCPSocketBind.msg", nil), portNo];
		[alert runModal];
		return self;
	}
...
- (NSInteger)numberOfAttachmentsInMessageID:(NSNumber*)mid {
...
- (void)attachSendThread:(id)obj
{
	...
	ipAddr					= [[obj objectAtIndex:1] unsignedIntValue];
