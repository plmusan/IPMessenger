...
- (id)init
{
	...
	while (bind(sockUDP, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
		...
		alert.informativeText = [NSString stringWithFormat:NSLocalizedString(@"Err.UDPSocketBind.msg", nil), myPortNo];
...
- (void)sendMessage:(SendMessage*)msg to:(NSArray*)toUsers
{
	...
	if ([msg.attachments count] > 0) {
		...
		NSNumber*			messageID		= [NSNumber numberWithInteger:msg.packetNo];
		for (Attachment* info in msg.attachments) {
			info.fileID	= [NSNumber numberWithInteger:count];
			[buffer appendFormat:@"%ld:%@:%llX:%X:%X:",
								count,
								info.file.name,
								info.file.size,
	...
	for (UserInfo* info in toUsers) {
		...
		if (mid >= 0) {
			// 応答待ちメッセージ一覧に追加
			RetryInfo* retry = [RetryInfo infoWithMessageID:mid
													command:command
														 to:info
													message:msg.message
													 option:option];
			[sendList setObject:retry forKey:retry.identifyKey];
			// タイマ発行
...
- (void)retryMessage:(NSTimer*)timer
{
	...
	RetryInfo*	retryInfo	= [sendList objectForKey:retryKey];
	if (retryInfo) {
		if (retryInfo.retryCount >= RETRY_MAX) {
			...
			if (ret == NSAlertSecondButtonReturn) {
				// 再送キャンセル
				// 添付情報破棄
				[[AttachmentServer sharedServer] removeUser:retryInfo.toUser
												  messageID:[NSNumber numberWithInteger:retryInfo.messageID]];
				// 応答待ちメッセージ一覧からメッセージのエントリを削除
				[sendList removeObjectForKey:retryKey];
				return;
			}
			// リトライ階数をリセットして再試行
...
- (void)processReceiveMessage
{
	...
	_MSG_TRC(@"recvdata parsed");
	_MSG_TRC(@"\tfrom      = %@", [msg fromUser]);
	_MSG_TRC(@"\tpacketNo  = %ld", msg.packetNo);
	_MSG_TRC(@"\tcommand   = 0x%08lX", [msg command]);
	_MSG_TRC(@"\tabsence   = %s", ([msg absence] ? "YES" : "NO"));
	_MSG_TRC(@"\tsealed    = %s", ([msg sealed] ? "YES" : "NO"));
	_MSG_TRC(@"\tlockec    = %s", ([msg locked] ? "YES" : "NO"));
	_MSG_TRC(@"\tmulticast = %s", ([msg multicast] ? "YES" : "NO"));
	_MSG_TRC(@"\tbroadcast = %s", ([msg broadcast] ? "YES" : "NO"));
	_MSG_TRC(@"\tappendix  = %@", [msg appendix]);
		...
		if ([msg hostList]) {
			...
			for (i = 0; i < [userArray count]; i++) {
				...
				if (![config matchRefuseCondition:newUser]) {
					_MSG_DBG(@"        > Append User([%d/%ld] %@)", i, userArray.count, newUser.summaryString);
					[userManager appendUser:newUser];
				}
			}
		}
		if ([msg hostListContinueCount] > 0) {
			_MSG_DBG(@"        > Send IPMSG_GETLIST(%d)", msg.hostListContinueCount);
			// 継続のGETLIST送信
	...
	case IPMSG_SENDMSG:		// メッセージ送信パケット
		...
		[(AppControl*)[NSApp delegate] receiveMessage:msg];
		break;
	case IPMSG_RECVMSG:		// メッセージ受信確認パケット
		...
		[sendList removeObjectForKey:[RetryInfo identifyKeyForMessageID:[appendix integerValue] to:fromUser]];
		break;
	...
	case IPMSG_GETINFO:		// 情報取得要求
		...
		_MSG_DBG(@"        > Response VersionInfo(%@)", version);
		[self sendTo:fromUser
		   messageID:-1
	...
	case IPMSG_ANSPUBKEY:
		_MSG_DBG(@"command=IPMSG_ANSPUBKEY:%@", appendix);
		_MSG_DBG(@"        > nop");
		break;
	/*-------- その他パケット／未知パケット（を受信） ---------*/
	default:
...
// 受信Rawデータの分解
+ (BOOL)parseReceiveData:(char*)buffer length:(size_t)len into:(IPMsgData*)data
{
	char* work	= buffer;
	char* ptr	= buffer;
	...
	*work = '\0';
	...
	// コマンド番号
	data->command = (UInt32)strtoul(ptr, &work, 10);
	if (*work != ':') {
		return NO;
	}
	ptr = work + 1;
