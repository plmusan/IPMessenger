...
- (DownloadResult)receiveFrom:(int)sock to:(void*)ptr maxLength:(size_t)len;
- (void)incrementNumberOfFile;
- (void)incrementNumberOfDirectory;
- (void)newFileDownloadStart:(NSString*)fileName;
- (void)newDataDownload:(size_t)size;
@end
...
//		DBG(@"dir:recv Header=%s", buf);
		...
		if (remain > 0) {
			...
			while (remain > 0) {
				...
				if (result != DL_SUCCESS) {
					ERR(@"dir:file receive error(%d,remain=%lu)", result, remain);
					break;
				}
				[self newDataDownload:size];
				remain -= size;						// 残りサイズ更新
