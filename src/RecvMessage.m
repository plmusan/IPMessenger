...
- (id)initWithBuffer:(const void*)buf length:(NSUInteger)len from:(struct sockaddr_in)addr
{
	...
	TRC(@"start parsing(buf=0x%08lX,len=%lu)--------", (unsigned long)buf, len);
	...
	if (len <= 0) {
		ERR(@"parameter error(len is %lu)", len);
		[self release];
		return nil;
	}
	...
	if ((len + 1) - (strlen(buffer) + 1) > 0) {
		...
		TRC(@"\tsubMessage   =\"%s\"(len=%ld[%lu,%ld])", subMessage, (len + 1) - (strlen(buffer) + 1), len, strlen(buffer));
		if ((len + 1) - (strlen(buffer) + 1) - (strlen(subMessage) + 1) > 0) {
			subMessage2 = &subMessage[strlen(subMessage) + 2];
			TRC(@"\tsubMessage2  =\"%s\"(len=%ld[%lu,%ld,%ld])", subMessage2, (len + 1) - (strlen(buffer) + 1) - (strlen(subMessage) + 1), len, strlen(buffer), strlen(subMessage));
		}
	}
