...
- (id)initWithRecvMessage:(RecvMessage*)msg {
	...
	// 重要ログボタン設定
	altLogButton.image.template = YES;
	...
	if (sender == attachSaveButton) {
		...
		[op beginSheetModalForWindow:window completionHandler:^(NSInteger result) {
			if (result == NSModalResponseOK) {
				...
				// シート表示
				[window beginSheet:attachSheet completionHandler:^(NSModalResponse returnCode) {
...
- (IBAction)openSeal:(id)sender {
	if ([recvMsg locked]) {
		...
		// シート表示
		[window beginSheet:pwdSheet completionHandler:^(NSModalResponse returnCode) {
			if (returnCode == NSModalResponseOK) {
				dispatch_async(dispatch_get_main_queue(), ^{
					// 封書消去
					[sealButton removeFromSuperview];
					messageArea.hidden = NO;
...
- (IBAction)okPwdSheet:(id)sender {
	...
	[window endSheet:pwdSheet returnCode:NSModalResponseOK];
}

// パスワード入力シートキャンセルボタン押下時処理
- (IBAction)cancelPwdSheet:(id)sender {
	[window endSheet:pwdSheet returnCode:NSModalResponseCancel];
}
...
- (void)downloadSheetRefresh:(NSTimer*)timer {
	if (attachSheetRefreshTitle) {
		...
		NSInteger index	= [downloader indexOfTarget] + 1;
...
- (void)downloadWillStart {
	attachSheetCancelButton.enabled = YES;
...
- (void)downloadDidFinished:(DownloadResult)result {
	attachSheetCancelButton.enabled = NO;
	...
	dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 500 * NSEC_PER_MSEC), dispatch_get_main_queue(), ^{
		[window endSheet:attachSheet returnCode:NSModalResponseOK];
...
- (id)tableView:(NSTableView*)aTableView
	...
	if (aTableView == attachTable) {
		...
		fileWrapper		= [[NSFileWrapper alloc] initRegularFileWithContents:[NSData data]];
