...
- (id)init
{
	...
	if (self) {
		...
		iconToggleTimer				= nil;
		iconNormal					= [[NSImage imageNamed:@"AppIcon"] retain];
		iconNormalReverse			= [[NSImage imageNamed:@"AppIcon_Reverse"] retain];
		iconAbsence					= [[NSImage imageNamed:@"AppIcon_Absence"] retain];
		iconAbsenceReverse			= [[NSImage imageNamed:@"AppIcon_AbsenceReverse"] retain];
		lastDockDraggedDate			= nil;
		lastDockDraggedWindow		= nil;
		iconSmallNormal				= [[NSImage imageNamed:@"MenuIcon"] retain];
		iconSmallNormal.template	= YES;
		{
			NSImage* tintImg = [iconSmallNormal copy];
			[tintImg lockFocus];
			[[NSColor alternateSelectedControlColor] set];
			NSRectFillUsingOperation(NSMakeRect(0, 0, tintImg.size.width, tintImg.size.height), NSCompositingOperationSourceAtop);
			[tintImg unlockFocus];
			tintImg.template = NO;
			iconSmallNormalReverse  = tintImg;
		}
		iconSmallAbsence			= [iconSmallNormal retain];
		iconSmallAbsenceReverse		= [iconSmallNormalReverse retain];
	}
